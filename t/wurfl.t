...
my $wurfl = eval {
    ...
    if ( defined $ENV{WURFL_URL} ) {
        ...
        ok( ( $WURFL_URL && -e $WURFL_URL ),
            "WURFL_URL is not set, so cloned something from github" );
    }
...
SKIP: {
    ...
    if ( !defined $ENV{WURFL_URL} || $ENV{WURFL_URL} !~ /^[a-z]+:\/\//i ) {
        if ( defined $ENV{WURFL_URL} ) {
            ...
            my $wurfl_path = File::Spec->catfile( $wurfl_home, $wurfl_fname );

            ok( -e $WURFL_URL, "WURFL_URL exists on the filesystem" );
            print "#   Copying '$WURFL_URL' to '$wurfl_path'...\n";
            File::Copy::copy( $WURFL_URL, $wurfl_path ) or die $!;
