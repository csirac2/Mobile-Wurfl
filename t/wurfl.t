...
MySQL:
 WURFL_DB_USER=wurfl WURFL_DB_PASS=wurfl WURFL_DSN=DBI:mysql;host=localhost;database=wurfl prove --verbose t/

SQLite:
 WURFL_DSN=DBI:SQLite;dbname=/tmp/wurfl.sqlite.db prove --verbose t/
...
my @t_path_dirs = File::Spec->splitdir($t_path);
my $long_user_agent = {
...
my $wurfl = eval {
    my %opts = (
        ...
        #  . File::Spec->catfile( $wurfl_home, 'wurfl.sqlite.db' ),
        #db_username => '',
        #db_password => '',
        #verbose => 2,
    );
    ...
    if ( !defined $ENV{WURFL_URL} ) {
        ...
        ok( ( $opts{wurfl_url} && -e $opts{wurfl_url} ),
            "WURFL_URL is not set, so improvise a source .xml" );
    }
...
exit unless $wurfl;
ok( eval { $wurfl->create_tables(); 1 }, "create db tables: $@" );
SKIP: {
    ...
    if ( !defined $ENV{WURFL_URL} || $ENV{WURFL_URL} !~ /^[a-z]+:\/\//i ) {
        if ( defined $ENV{WURFL_URL} ) {
            ...
            my $wurfl_path = File::Spec->catfile( $wurfl->{wurfl_home}, $wurfl_fname );
        ...
        ok( $wurfl->rebuild_tables(),
            "rebuild_tables because WURFL_URL not set or not really a URL" );
        skip( "WURFL_URL unset or not a URL, can't test get_wurfl()/update()",
            5 );
    }
...
my %capabilities; for my $group ( @groups )
{
...
TODO: {
    local $TODO ='Check desktop UAs actually exists in the DB';
...
sub device_capabilities_timing_tests {
    ...
    $count ||= 100;
    ok(
        defined $canon_ua && length($canon_ua),
        "got a canonical ua for $annotation"
    );
    print '#   input ua len: ' . length($ua) . "\n";
    print "#   canonical_ua: '$canon_ua'\n" if $canon_ua;
    my $results = Benchmark::timethese(
        $count,
        {
            $method1 =>
